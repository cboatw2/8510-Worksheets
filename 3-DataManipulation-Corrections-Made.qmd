...
```{r}
locations <- gayguides %>% select(city, Year)
locations
```

(@) What did you do to save the data to a new data frame? Why? 

> I assigned the selected columns to a new dataset. I believe this works because of the pipe function (%>%) in the tidyverse - in other words, rather than creating a function to run through the entire dataset and subset into a new dataset like we did last week, the %>% works as a short hand function in tidyverse.
...
```{r}
gayguides %>% filter (city == "Greenville" & state == "SC" & Year %in% c(1975:1980))
```
...
```{r}
gayguides %>% 
  separate_rows(amenityfeatures) %>%
  distinct(amenityfeatures) %>%
  arrange(amenityfeatures) 
```
...
BostonWomenVoters %>% mutate(Year.of.Birth = 1920 - Age)
```
...
```{r}
data("gayguides")
gayguides %>% mutate(Location = paste0(city, ", ", state))
```
...
statepopulations %>%
  select (X1850, STATE) %>%
  arrange (desc(X1850), .by_group = TRUE)
```
...
```{r}
gayguides %>% 
    select(state, title, Year) %>%
    group_by(state, Year) %>%
    summarize(numberOfLocations = n())
```
...
gayguides %>%
   ...
   group_by (city) %>%
  filter (Year == 1970:1975 & state == "SC") %>%
  summarize (averageNumberOfLocations = mean(n(title)))
```
```{r}
gayguides %>%
filter(gayguides, state == "SC") %>% filter(Year == 1970:1975) %>% group_by (city, Year) %>% summarize(count = n()) %>% summarize(avg = mean(count))
```
...
>Joined the new dataset, regions, with the gayguides dataset. I joined the state column of gayguides with State.Code of regions. Both datasets appear and regions appears 3 times as x, y and normally because it is attempting to match all variables of the dataset.
...
```{r}
#gayguides %>%
  #select (Year, state, title) %>%
  #group_by (state, filter (Year = 1970:1980)) %>%
  #summarize (count(n))
```
...
>It transformed the sc.parks dataset by keeping the city through the type_of_worker columns, but creating a new year column and sending what were column title names into the cells of that column and then creating a new count column and sending the values of the old year columns to that.
...
```{r}
#What is the average lifespan of a judge by gender?
```
...
```{r}
#judges %>%
  #select (Birth.Year, Death.Year, Gender) %>% #grabs the columns needed
  #group_by (Gender) %>% #sets up output by Gender
  #mutate (Age = Death.Year - Birth.Year) #attempting to create a new Age column through subtracting Birth.Year from Death.Year
```

(@) Now ask a question of the `gayguides` data (or another dataset of your choice). What is the question you are asking? 
```{r}
#In the datast rec.spaces, what year had the highest number of occurrences for each type of equipment?
```
...
gayguides_lookup <- function (state, year) {
  
}
```

```{r}
gg.filter <- function (gg.year, gg.state) {
  gayguides %>% select(Year
                       , state, title, type) %>% filter(Year == gg.year & state == gg.state)
}
```
```{r}
gg.filter(1980, "NY")
