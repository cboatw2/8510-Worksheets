...
>What is the number of Boston women votes by the state of their birth state?

(@) Now write the code to answer that question and use comments (`#`) to explain your thinking as you go.
```{r}
data("BostonWomenVoters")

birth.state <- BostonWomenVoters %>% select(State.or.Province.of.Birth) %>% summarise(count(n))

ggplot(birth.state, aes(x = State.or.Province.of.Birth, y = count)) + geom_col()
```
