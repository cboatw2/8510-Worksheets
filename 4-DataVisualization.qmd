...
ggplot(rel.inst, aes(x = state, y = count)) + geom_point() + geom_segment(aes(x=state, xend=state, y=0, yend=count))
...
>What is the number of Boston women voters by their birth state?
...
birth.state <- BostonWomenVoters %>% group_by(State.or.Province.of.Birth) %>% count () %>% filter(n > 0) %>% print #creates a function to group the dataset by the voters' state/province of birth and then to count each state/province

ggplot(birth.state, aes(x = State.or.Province.of.Birth, y = count(n))) + geom_col()
```
...
southern.risk <- gayguides %>% group_by(Year) %>% filter(state == "SC" | state == "NC" | state == "GA") %>% filter(str_detect(gayguides, "AYOR")) %>% summarize(count = n()) %>% print
...
StudentByState <- BostonWomenVoters %>% filter(Country.of.Birth == "United States") %>% filter(Occupation == "Student") %>% group_by(State.or.Province.of.Birth)

ggplot(StudentByState, aes(y = count())) +
  geom_histogram()
```

```{r}
 %>% summarize(count(n = State.or.Province.of.Birth))

```

(@) Plot 1 explanation: 
>Histogram indicating number of women students in each state in the United States.
