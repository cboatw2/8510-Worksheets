...
southern.risk <- gayguides %>% select (Year, description, state) %>% group_by(Year) %>% filter(state %in% c("SC", "GA", "NC")) %>% filter(description == "Religious Institution") %>% summarize(count = n()) %>% print
