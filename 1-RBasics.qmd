...
> The paste function places strings together seperated by a space. Whatever character vectors are placed inside of the quotation marks within the paste function are generated into a single output of characters.
...
>5 is equal to 10 returns FALSE
>3 is less than 4 returns TRUE
>5 is not equal to 10 returns TRUE
>5 is not equal to 5 returns FALSE
>3 is equal to 4 or 3 returns TRUE
>3 is not equal to 4 and 3 is not equal to 5 returns TRUE
...
```{r}
NumbersOfDaysUntilThePoolOpens <- 105
```

(@) Can you assign a string to a variable?

```{r}
"All My Children" <- 12789
```
...
NumberOfTilesLong <- 8
NumberOfTilesWide <- 4
NumberOfTilesLong * NumberOfTilesWide
```
...
>The four numbers 2, 4, 6, 8 were stored as the 'my.numbers' vectors. When my.numbers was multipled by 2, each of the stored numbers was multipled by 2. So running 'my.numbers * 2' returned 4, 8, 12, 16. This did not change the original numbers stored in the vector.
...
>The number stored in the third location in the vector 'my.numbers', 6, is multipled by 2. Because the previous code did not alter the original numbers stored in my.numbers the answer returned is 12.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
NFLPlayoffTeams2024 <- c("Kansas City Chiefs", "Baltimore Ravens", "San Francisco 49ers", "Detroit Lions")
```

```{r}
NFLPlayoffTeams2024 [c(1,3)]
```
>I used [c()] to recall and combine multiple items in the vector list. In this case, the two NFL teams advancing to Super Bowl LVIII.
...
>It rounds down the decimal to the nearest whole number.
...
>Row z takes the number in each row of column x, multiples by 2, then adds the number in row y. The solution is stored in the corresponding row in column z.
...
>Contains transcribed information from Bob Damron's Address Books from 1965-1985
...
>I googled 'how list the first 10 rows in rstudio' which gave me the answer for how to modify the number of rows displayed. Tail is the same as head and I didn't need a modifier because the default is 6 rows.
...
>'gayguides' is the dataset, '$' extracts a specific subset of the dataset, 'title' is the specific subset we're wanting to see, '[500]' is the location of the subset. In other words, this is a search function that is returning the contents of the cell in the title column of the 500th row.
...
```{r}
statepopulations$X1790
```

```{r}
statepopulations$STATE
```
...
>This is a dataset listing the U.S. census data by state or territory from 1790 thru 2020 (although the title only includes thru 2010). States without census data for a given year returns NA. The observations (Persons in the Military and some of the territories) that do not hold any numeric data also return NA in the STATEFP column - does this prevent results from being skewed?
